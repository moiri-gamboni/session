...
install.sh [--bindir DIR] [--name NAME] [--force-link] [--dry-run] [--no-rewake]
           ...
           [--auto-switch | --no-auto-switch] [--switch-notify PATH]
```
...
Eleven checks, each asking whether the thing works *on this machine* and printing the fact it decided on. Four further lines appear only when they have something to report: `session.conf` when that file is broken or ignored, `warn pct` when `USAGE_WARN_PCT` is not a whole number, `credentials` when the live access token is empty, and `vault` when a vaulted entry carries no identity of its own, one that does not derive the name it is filed under, or the access token that is currently installed while filed under another login's name. Three states, because "not yet" and "wired wrong" have different remedies: **ok**, **pending** (nothing has produced it yet — usually "interact once"), **FAIL** (wired wrong, and the line says what). Only a FAIL exits 1. Case 19 in `tests/session.test.sh` pins all three; the `doctor:` block at that suite's tail pins check 11 and the `warn pct` and `vault` lines.
...
| `.probe.<pid>/`, `.ident.<pid>/`, `.dry.<pid>/` | scratch directories a decision removes on its way out: the parallel probe's per-login result files, one vault entry's identity, and a `--dry-run`'s discarded cache writes. One left behind is a process that died mid-decision; nothing reaps them, and each holds no token |
...
`--dry-run` prints a table above those lines — a `live=` row and one `cand=` row per login the decision is choosing among, each as `<login>/<tier>/probe|frozen/<5h>/<wk>/<fb>` — and writes no cache file, sends no notification and swaps nothing. The rows are not in rank order and the winner is named by `to=`, not by its position; a vault entry the screening below rejected has no row at all, which is the thing to know when asking why it will not move to one particular login. Its `ev=` reads `dry-run` and it exits 3, like any hold. The one file it does create is `switch.lock`, because taking the mutex is what serialises it against a real decision.
...
**Each login is probed with the token that serves it**, against the same `GET https://api.anthropic.com/api/oauth/usage` the `FABLE` column uses. The live login goes first, with the live `.credentials.json`, since that is the token actually serving requests; the candidates follow in parallel, 5 s each. A vault entry is screened out before any token of its own is sent when it carries no usable access token, when its `expiresAt` reads 0, when its access token is the one already live, or when its own `oauthAccount` does not derive the name it is filed under. Such an entry is silently not a candidate: `session doctor`'s `switcher` line counts how many vault entries carry a token at all, and its `vault` lines name both the entry whose own identity does not derive its filename and the entry holding the **installed** access token under another login's name. The second is what the swap's two-file window leaves behind — switching to it would install the credential already in place, and the swap's post-condition, which compares against the entry, would confirm a switch that never happened. Skipping it silently costs the other direction: the live name is skipped too, so every decision after it holds on no candidate, which is why that `vault` line exists and names the file, the mismatch and the verb that ends it. The probe's state decides whether a login's figures are fresh or frozen, and is classified on the HTTP status alone:
...
**Announcing the move.** `SESSION_SWITCH_NOTIFY`, when it names an executable, is run with one argument — for a switch, a sentence naming both logins and what the new one serves — on a switch and on a blank-credential refusal, never on a hold. Holds are the normal outcome and announcing them is around nine high-priority pushes a day through a Fable-capped week. The **switch** notification fires after the lock is released, never inside it, and there is no `timeout` anywhere: `lock_run` execs its command, so a detached grandchild inherits the lock descriptor and holds the mutex for as long as it lives — measured on both backends, and closing its stdin does not release it — and a runaway notifier inside the lock would block every later decision. `timeout` would not have bounded it either: macOS does not ship the command at all, so the guard would have been a no-op on half the port.

**The refusal announces itself from one of its two callers only**, for the same reason. A blank live credential is refused wherever it is met: from the statusline's autosave, where nobody is watching and the push is the whole point, and from the swap's "never lose the login being replaced" save — which a decision runs inside the lock, where a hanging notifier pins it exactly as a hanging switch notification would. The blank-credential shape is also the one that reaches it: the decision that has to move the box is the decision that finds nothing worth vaulting behind it. So the refusal met while swapping records its row and sends nothing — on `session account use` as much as on a decision, and there the row is the *whole* record, because the swap discards the save's stderr too. The **row** is written on every path: it is what `doctor` reads, and what the refusal's own cooldown is measured against — which means a silent row spends the notification budget like a sent one, and an autosave that refuses the same login inside the next 900 s writes nothing and sends nothing.
...
1. Append `SESSION_AUTO_SWITCH=off` to `<config dir>/session.conf`. The conf is read on every invocation and each decision is a fresh child, so the next cap death already sees it; nothing restarts. `install.sh --no-auto-switch` records the same line, and a recorded line survives every later re-run of the installer — `--auto-switch` is what puts the machine back.
...
**Why a quiet week is not a fault.** The feature was designed against a record in which a capped session almost always had another login with generic-window headroom to move to. That regime has since moved: weekly counts of generic-window against model-scoped cap deaths ran 137/17, then 89/50, 6/27, 15/48, and then 0/67. In a week where every login's Fable quota is spent, every login sits at tier 1 or below, nothing climbs, and the decision holds — which is the policy working, not an install that is broken. What the same record says is worth most is the other trigger rather than the cap one: a single interactive authentication failure landed five minutes into a blank-credential outage in which every session failed for ten and a half hours, with no recent switch to be in probation from, and one decision would have ended it. The crux that would move the cap half back to the centre is Fable quota ceasing to be the binding constraint — a plan change, a model change, or the scoped cap widening.
...
**Only the wake-up itself writes to that entry's stderr.** Because exit 2 hands stderr to the model, stderr on `--rewake-waiter` is a message channel into a live session and not a log: the entry discards fd 2 at the top of the script — ahead of the lib sourcing, which is itself one of the producers, alongside the statusline cache read — and parks the real one on fd 9, whose only writer is `rewake_done`. A refusal is not a wake-up either. An unreadable guard spec, an unreadable `USAGE_WARN_PCT` or an argument this build does not know exits 2 with a message on the entry it was given to, and on this one exits 0 with nothing, arming no waiter. What still makes such a value visible is the *synchronous* `--hook` entry, which refuses on every prompt and blocks it; `session doctor` adds a line for the warn threshold alone, having dispatched long before the guard specs are read. Both halves are box-wide: one malformed line in `session.conf` is read by every session on the machine, on every prompt.
...
- **Locking** uses `flock(1)` where it exists and perl `flock` otherwise. The perl path sets `$^F=10`, which is load-bearing: perl sets `FD_CLOEXEC` on descriptors above `$^F` (default 2), so without it the lock is dropped by the `exec` and there is no mutual exclusion at all — a failure that looks exactly like success. Case 12 pins it by showing the second caller exit 75 while the lock is held. Busy is 75 wherever the backend can say so; busybox `flock` has no `-E` and exits 1, and callers treat any non-zero as "skip". **A lock file that cannot be opened is not busy**, and the two backends do not test the same thing: `flock(1)` opens `O_RDONLY|O_CREAT` and answers out of `<sysexits.h>` — 66 for an existing file it cannot open, 73 where it cannot create a missing one, 71 on resource exhaustion — while the perl path opens for append, so a read-only or foreign-owned lock file stops it at 66 where `flock(1)` takes the lock and runs. That makes the perl path, i.e. macOS, where an unopenable `switch.lock` actually bites; none of those codes is in the busy pair, and `session account auto` turns 66 into a `lock-unopenable` hold rather than passing it back.
