...
| `.probe.<pid>/`, `.dry.<pid>/` | scratch directories a decision removes on its way out: the parallel probe's per-login result files, and a `--dry-run`'s discarded cache writes. One left behind is a process that died mid-decision; nothing reaps them, and each holds no token |
