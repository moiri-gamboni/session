...
Everything is one environment variable with a default, defined once in `lib/common.sh`.
...
**The swap is read back before `use` reports it.** After writing, the installed `claudeAiOauth` is compared with the vault entry's, out of `.credentials.json` rather than out of `.claude.json` — the identity file is a head start for the next statusline render, not the source of truth, and a concurrent session can rewrite it from memory. A vault entry carrying no `claudeAiOauth` fails the same check, so a blank credential is never installed quietly. A mismatch exits 1 with a message naming the file, and neither the confirmation nor the table is printed; the Fable refetch does not run. The comparison is a boolean, so no token value is ever rendered. **`.claude.json` is written before the check**, so after a mismatch it names the login whose credential did not land — `session account list` marks that login `live` until a swap succeeds.
...
One path worth knowing. When a turn dies on a usage cap and neither window has warned, the waiter arms on whichever reset is still ahead — the 5-hour one first, the weekly otherwise. With a stale cache whose 5-hour reset has already passed, that means the weekly, which can be up to seven days out. This is deliberate (a `rate_limit` death with a stale 5-hour reset genuinely can be the weekly cap) and predates the port, but it is the one path where the waiter can outlive its usefulness by days.
