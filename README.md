...
The decision verb's own refusals (`off`, `bad-mode`, `no-credentials`, `too-few-logins`), its `not-writable` hold and its argument errors (`bad-threshold`, `bad-trigger`, `bad-option`) reach its `k=v` output and never this file: each returns before the cooldown gate, and `not-writable` is the case where there is nowhere to write.
...
Exit codes: **0** switched, **3** held, **4** refused, **5** error. **Never 1** — 1 and 75 are the two locking backends' busy codes, so a caller reads either as "another decision holds the lock" and falls through with no retry and no look at the log, whose newest row describes some earlier decision. A verb that never ran at all wears its backend's code instead of a decision's: 66 when the lock file cannot be opened, a state `session doctor` reports on its own line. Every outcome prints the same six lines:
...
**Each login is probed with the token that serves it**, against the same `GET https://api.anthropic.com/api/oauth/usage` the `FABLE` column uses. The live login goes first, with the live `.credentials.json`, since that is the token actually serving requests; the candidates follow in parallel, 5 s each. A vault entry is screened out before any token of its own is sent when it carries no usable access token, when its `expiresAt` reads 0, when its access token is the one already live, or when its own `oauthAccount` does not derive the name it is filed under. Such an entry is silently not a candidate: `session doctor`'s `switcher` line counts how many vault entries carry a token at all, and its `vault` line names one whose own identity does not derive its filename. The live-token case is the one `doctor` cannot see — that entry's identity does derive its filename — and it is what the swap's two-file window produces: switching to it would install the credential already in place, and the swap's post-condition, which compares against the entry, would confirm the switch. The probe's state decides whether a login's figures are fresh or frozen, and is classified on the HTTP status alone:
...
- **Locking** uses `flock(1)` where it exists and perl `flock` otherwise. The perl path sets `$^F=10`, which is load-bearing: perl sets `FD_CLOEXEC` on descriptors above `$^F` (default 2), so without it the lock is dropped by the `exec` and there is no mutual exclusion at all — a failure that looks exactly like success. Case 12 pins it by showing the second caller exit 75 while the lock is held. Busy is 75 wherever the backend can say so; busybox `flock` has no `-E` and exits 1, and callers treat any non-zero as "skip". A lock file that cannot be **opened** is 66 on both backends — `flock(1)`'s `<sysexits.h>` code for it, which the perl path matches — and deliberately outside the busy pair: a caller reading busy falls through silently, so an unopenable `switch.lock` reported as busy would disable automatic switching for ever without a row or a message.
