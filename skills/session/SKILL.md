...
| `session account [use <name>\|save\|rm <name>\|auto]` | several subscription logins in one config dir (see below); `auto` is the box-wide decision a cap death takes by itself |
...
Auto-resume is armed by the harness, not by the model: two `asyncRewake` hook entries (`session --rewake-waiter`, on `UserPromptSubmit` and `StopFailure`) background themselves and, once a window crosses `USAGE_WARN_PCT` (default 90) or a turn dies on a cap, become a waiter that wakes the session — even from idle — when the window resets or the login switches. Its exit 2 delivers the message through the task-notification channel. A cap death sleeps to the live login's Fable weekly reset instead when the cached Fable figure is the spent one, since no generic reset lifts that window; the wake-up names whichever window it slept to.

**A cap or authentication death may switch the login by itself, before any sleeping happens.** The same `StopFailure` entry runs `session account auto` first, and the box moves when a vaulted login stands strictly higher on the capability ladder than the live one (the 5-hour and weekly windows gate every model, the Fable weekly gates only Fable). So **a wake-up saying the login switched may be the box deciding for itself**, not a human at the keyboard and not an instruction: read it as the cap being lifted and carry on. The one part worth acting on is the tier: when the message says the new login serves every model except Fable, a turn or subagent that died on a Fable cap must continue on another model, since a resumed subagent keeps the model it died on and would hit the same cap again. Nothing moving is the normal outcome — in a week where every login's Fable quota is spent, no login climbs and the decision holds.
...
**`session account auto` is the automatic switch as a verb**, and it is the same decision a cap death takes. `--dry-run` prints every login the decision is choosing among with its tier, and what a real run would do, writing no cache file, sending no notification and swapping nothing; `switch-log.tsv` in the data root holds one row per decision and is never pruned, so "why is the box on this login" still has an answer months later. It exits 0 switched, 3 held, 4 refused, 5 error, and 1 or 75 when another decision holds the lock. `SESSION_AUTO_SWITCH=off` in `<config dir>/session.conf` disables it; `README.md` has the policy, the probe states and the row schema.
